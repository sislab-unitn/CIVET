...
def main():
    ...
    prop_vairations = list(product(SHAPES, COLORS, SHEENS, SIZES, positions))
    ...
    for name, orders in zip(
        ["shape", "color", "sheen", "area", "area_vert", "area_hor"],
        [shape_orders, color_orders, sheen_orders, area_orders, area_vert_orders, area_hor_orders]
    ):
        print("-"*40)
        print(name)
        c = Counter(orders)
    ...
    for stim_n, (variation, shape_ord, color_ord, sheen_ord, area_ord, area_vert_ord, area_hor_ord) in tqdm(enumerate(variations), desc=f"Generating stimuli", unit="stimuli", total=len(variations)):
        ...
        shape, color, sheen, size, position = variation
        ...
        for prop, val_order in zip(["shape", "color", "sheen"], [shape_ord, color_ord, sheen_ord]):
