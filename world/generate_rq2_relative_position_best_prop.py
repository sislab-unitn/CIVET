...
from questions import PROP_VALUES, PROPERTIES_ORDER, absolute_position_question, prop_question, properties_questions, relative_position_question
...
def main():
    ...
    # add size
    size_pairs = list(product(["large"], ["large"]))
    ...
    for stim_n, (variation, relative_rod) in tqdm(enumerate(variations), desc=f"Generating stimuli", unit="stimuli", total=len(variations)):
        ...
        # A w.r.t. B
        questions[stim_n][e1.id]["position_relative"] = relative_position_question(e1, e2, val_order=relative_rod, properties=question_properties)
        # B w.r.t. A
        questions[stim_n][e2.id]["position_relative"] = relative_position_question(e2, e1, val_order=relative_rod, properties=question_properties)
